...
def get_post(id):
    conn = db_conn()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM tbl_berita WHERE id = ?', (id,))
...
def search():
    ...
    conn = db_conn()
    cursor = conn.cursor()
    query = ("SELECT * from tbl_berita where judul like ? ORDER BY id")
...
def update():
    ...
    query = "UPDATE tbl_berita SET judul = ?, tanggal = ?, isi = ? WHERE id = ?"
...
def store():
    ...
    data = fetchAll()
    id = data.pop()[0]+1
    query = "INSERT INTO tbl_berita (id, judul, tanggal, isi) values (?, ?, ?, ?)"
...
def delete():
    id = request.args.get('id')
    query = "delete from tbl_berita where id = ?"
